...
function DownloadModal({ open, onClose, deviceId }) {
  ...
  const downloadCSV = () => {
    ...
    window.open(
      `${DVSG_BASE}/device/${encodeURIComponent(deviceId)}/logs/download?${params.toString()}`,
      "_blank"
    );
    onClose();
  };
      ...
      >
        ...
        <div style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
            ...
            <button onClick={downloadCSV} style={{ ...styles.btnInfo, padding: "10px 20px" }}>
              📥 Download CSV
            </button>
          </div>
        </div>
                ...
                <tr key={r.key} style={{ transition: "background 0.2s" }}>
                  ...
                  <td style={{ ...styles.td, textAlign: "right" }}>
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", flexWrap: "wrap" }}>
                      ...
                      {/* UPDATED: opens temperature chart */}
                      <button
